...
class TaskDetailViewController: UIViewController {
    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        ...
        if PHPhotoLibrary.authorizationStatus(for: .readWrite) != .authorized {
            ...
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { [weak self] status in
                switch status {
                case .authorized:
                    // The user authorised access to their photo library
    ...
    private func presentImagePicker() {
        ...
        // Request the original file format. Fastets method as it avoids transcoding
        ...
        // Set the picker delegates so we can recieve whatever image the user picks
    ...
    func updateMapView() {
        ...
        //get the coordinates from the image location. this is the latitude / longitude of the location.
        let coordinate = imagelocation.coordinate
        
        // Set the maps view's region based on the coordinate of the image
        // The span represents the map's "zoom level". A smaller value yields a more "zoomed in" map area, while a larger value yields a more "zoomed out" area
...
extension TaskDetailViewController: PHPickerViewControllerDelegate {
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        provider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
            ...
            // UI updates should be done on main thread, hence the use of `DispatchQyeye.main.async`
            DispatchQueue.main.async { [weak self] in
                // Set the picker image and location on the task
